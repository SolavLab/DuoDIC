...
for ip=1:nPairs
    ...
    if strcmp(faceMeasureString,'pairInd')
        hp(ip)=gpatch(Fnow,Pnow,Colors(ip,:),Colors(ip,:),optStruct.FaceAlpha);view(180,-75); hold on
    else
        hp(ip)=gpatch(Fnow,Pnow,CFnow,optStruct.lineColor,optStruct.FaceAlpha);view(180,-75); hold on
    end
end
