...
switch pointMeasureString
    ...
    case {'DispMgn'}
        for ip=1:nPairs
            for it=1:nFrames
                PC{ip,it}=DIC3DAllPairsResults{ip}.Disp.DispMgn{it};
            end
        end
        PCmat = cell2mat(PC);
    ...
    case {'DispX'}
        for ip=1:nPairs
            for it=1:nFrames
                PC{ip,it}=DIC3DAllPairsResults{ip}.Disp.DispVec{it}(:,1);
            end
        end
        PCmat = cell2mat(PC);
    ...
    case {'DispY'}
        for ip=1:nPairs
            for it=1:nFrames
                PC{ip,it}=DIC3DAllPairsResults{ip}.Disp.DispVec{it}(:,2);
            end
        end
        PCmat = cell2mat(PC);
        PClimits=[min(PCmat(:)) max(PCmat(:))];
    ...
    case {'DispZ'}
        for ip=1:nPairs
            for it=1:nFrames
                PC{ip,it}=DIC3DAllPairsResults{ip}.Disp.DispVec{it}(:,3);
            end
        end
        PCmat = cell2mat(PC);
        PClimits=[min(PCmat(:)) max(PCmat(:))];
