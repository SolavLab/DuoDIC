...
%% Face-edge connectivity - edges are two points that are determend in Face each face has 3 edges
E=patchEdges(F,0); %The non-unique edge set
E_sort=sort(E,2); %Sorted in column dir so 1 2 looks the same as 2 1
indEdges=sub2indn(numVertices*ones(1,2),E_sort); %Create "virtual" indices( places of edges in a matrix(nXn) we will call edgematrix)
[~,ind1,indEdges_2]=unique(indEdges); %Get indices for unique edges, indEgeds2 is the places of the same points?
E_uni=E(ind1,:); %Get unique edges (two edges in the spot will be eleminated)
ind_F_E=reshape(indEdges_2,numFaceVertices,numFaces)'; %Reshape to get results in a matrix of faces(3 points)
...
ind_E_F=sparse(ind_F_E(:),ind,ind,numEdges,numFaces,numel(ind)); %Create sparse form of connectivity matrix (numbering the 
ind_E_F=sort(ind_E_F,2,'descend'); %Sort the sparse array
ind_E_F=full(ind_E_F(:,[1 2])); %Keep relevant columns, convert to full array (What is in side? the places of the edges by order?)
...
[~,J,~] = find(ind_V_F);%col where no zero 
